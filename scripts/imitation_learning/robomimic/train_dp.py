...
"""
The main entry point for training robomimic policies from pre-collected data.

This script loads dataset(s), creates a robomimic model, and trains it.
It supports training on IsaacLab environments with multiple robomimic algorithms
(e.g. diffusion_policy, act, bc_rnn).

Args:
    algo: Name of the algorithm to run (e.g. 'diffusion_policy', 'act').
...
# Add ACT to Python path only when ACT algorithm is requested.
import sys
if "--algo" in sys.argv:
    try:
        algo_idx = sys.argv.index("--algo")
        if algo_idx + 1 < len(sys.argv) and sys.argv[algo_idx + 1].lower() == "act":
...
def train(config: Config, device: str, log_dir: str, ckpt_dir: str, video_dir: str):
    """Train a robomimic model using the algorithm specified in config.
    ...
    # create model for the selected algorithm
    print(f"\n============= Creating {config.algo_name} Model =============")
    model = algo_factory(
...
def main(args: argparse.Namespace):
    """Train a robomimic model on a task.

    Args:
        args: Command line arguments.
    """
    # load config
    TEMPLATE_PATH = "/home/abc/IsaacLab/robomimic/robomimic/exps/templates/diffusion_policy.json"   # ← 根据你的实际路径修改
    # 或者用 site-packages 路径：
    

    print(f"【直接使用官方模板】 {TEMPLATE_PATH}")
    ...
    with config.values_unlocked():
        ...
        config.experiment.rollout.enabled = False   # 训练时关闭 rollout
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Name of the algorithm (e.g. 'diffusion_policy', 'act')."
    )
    
    parser.add_argument(
        "--log_dir", 
        type=str, 
        default="robomimic_diffusion_policy", 
