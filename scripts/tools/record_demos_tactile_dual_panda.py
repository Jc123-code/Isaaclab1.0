...
def main():
    ...
    env_cfg.viewer.eye = (0.47, 0.0, 6)     # 摄像机的位置（container 正上方）
    env_cfg.viewer.lookat = (0.47, 0.0, 1.05)  # 摄像机看的目标位置（container/杯子中心）
    ...
    def enable_gsmini_debug(env):
        ...
        for name, sensor in env.scene.sensors.items():
            if "gsmini" not in name:
                continue
            if not hasattr(sensor, "cfg") or not hasattr(sensor, "_prim_view"):
                continue
            for dt in [dt for dt in getattr(sensor.cfg, "data_types", []) if dt in ("marker_motion", "tactile_rgb")]: #触觉rgb图和标记图
                try:
                    sensor._prim_view.prims[0].GetAttribute(f"debug_{dt}").Set(True)
                except Exception:
                    pass
