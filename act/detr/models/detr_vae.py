...
class DETRVAE(nn.Module):
    ...
    def __init__(self, backbones, transformer, encoder, state_dim, num_queries, camera_names, action_dim=None):
        ...
        Parameters:
            ...
            action_dim: action dimension (if None, defaults to state_dim for backward compatibility)
        """
        super().__init__()
        
        # 向后兼容：如果没有提供 action_dim，使用 state_dim
        ...
        # 使用 action_dim 而不是硬编码的 state_dim
...
def build(args):
    # 使用 args 中的维度，如果不存在则使用默认值 14（向后兼容）
    state_dim = getattr(args, 'state_dim', 14)
    action_dim = getattr(args, 'a_dim', 14)
