...
def drawer_dragged(
    ...
    drawer_tf_data: FrameTransformerData = env.scene["table_frame"].data
    if drawer_tf_data.target_pos_w.shape[1] < 2:
        # 在 get_pen 任务中只存在 pen 目标，无 pen_holder，因此返回全 False
        return torch.zeros(env.num_envs, device=drawer_tf_data.target_pos_w.device, dtype=torch.bool)

    # 查看偏差
    dist_x = torch.norm(
        drawer_tf_data.target_pos_w[..., 0, [0]] - drawer_tf_data.target_pos_w[..., 1, [0]], dim=0
    )
    # 额外条件可扩展为 y/z
    dragged = dist_x > dist_threshold
