...
from isaaclab_tasks.manager_based.manipulation.push_plate_center import mdp
from isaaclab_tasks.manager_based.manipulation.push_plate_center.mdp import franka_pushplatecenter_events
from isaaclab_tasks.manager_based.manipulation.push_plate_center.pushplatecenter_env_cfg import PushPlateCenterEnvCfg
...
class EventCfg:
    ...
    init_franka_arm_pose = EventTerm(
        func=franka_pushplatecenter_events.set_default_joint_pose,
        mode="startup",
        params={
    ...
    reset_table_joint_pose = EventTerm(
        func=franka_pushplatecenter_events.reset_to_default_joint_pose,
        mode="reset",
        params={
            "default_pose": None,
            "asset_cfg": [SceneEntityCfg("table")],
    ...
    randomize_franka_joint_state = EventTerm(
        func=franka_pushplatecenter_events.randomize_joint_by_gaussian_offset,
        mode="reset",
        params={
            "mean": 0.0,
            "std": 0.02,
    ...
    reset_plate_pose = EventTerm(
        func=franka_pushplatecenter_events.reset_pose_to_default,
        mode="reset",
        params={
    ...
    reset_ReferencePoint_pose = EventTerm(
        ...
        params={
            "default_pose": torch.tensor([
                # 0.4, 0, 1.05,0.70711, 0.70711, 0, 0,   # 蓝书位置 + 姿态
                0.47, 0, 1.05,0.70711, 0.70711, 0, 0,   # 蓝书位置 + 姿态
                0, 0, 0, 0, 0, 0              # 静止
            ], device="cuda"),
            "asset_cfg": [SceneEntityCfg("ReferencePoint")],
        },
    )   

@configclass
class FrankaPushPlateCenterEnvCfg(PushPlateCenterEnvCfg):
    ...
    def _marker_cfg(self, gelpad_prim: str) -> FOTSMarkerSimulatorCfg:
        return FOTSMarkerSimulatorCfg(
            ...
            frame_transformer_cfg=FrameTransformerCfg(
                ...
                target_frames=[
                    FrameTransformerCfg.FrameCfg(
                        prim_path="/World/envs/env_.*/Table/plate",
                        name="switch",
                    )
                ],
                debug_vis=False,
