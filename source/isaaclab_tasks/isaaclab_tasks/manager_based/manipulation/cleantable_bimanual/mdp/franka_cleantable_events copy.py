...
def reset_to_default_joint_pose(
    ...
    if default_pose is not None:
        ...
        for pose, cfg in zip(poses, cfgs):
            asset: Articulation = env.scene[cfg.name]
            pose_tensor = torch.as_tensor(pose, device=env.device).view(1, -1)  # (1, DoF)
            dof = asset.data.default_joint_pos.shape[-1]
            assert pose_tensor.shape[-1] == dof, f"Pose dimension {pose_tensor.shape[-1]} != DoF {dof}"
            
            # 扩展到需要重置的环境数量
            num_reset = len(env_ids)
            joint_pos = pose_tensor.expand(num_reset, -1)  # 使用expand而不是repeat更高效
            joint_vel = torch.zeros(num_reset, dof, device=env.device)  # 创建零速度
            
            # 写入位置和速度到仿真
            asset.write_joint_state_to_sim(joint_pos, joint_vel, env_ids=env_ids)
            
            # 同时设置目标位置和速度（对于位置控制很重要）
        ...
        for cfg in cfgs:
            ...
            joint_vel = torch.zeros_like(joint_pos)  # 创建零速度
