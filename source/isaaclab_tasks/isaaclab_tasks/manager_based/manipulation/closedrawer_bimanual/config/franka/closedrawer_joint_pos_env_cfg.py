...
from isaaclab_tasks.manager_based.manipulation.closedrawer_bimanual import mdp
from isaaclab_tasks.manager_based.manipulation.closedrawer_bimanual.mdp import franka_closedrawer_events
from isaaclab_tasks.manager_based.manipulation.closedrawer_bimanual.closedrawer_env_cfg import CloseDrawerEnvCfg
...
class EventCfg:
    ...
    reset_table_joint_pose = EventTerm(
        func=franka_closedrawer_events.reset_to_default_joint_pose,
        mode="reset",
        params={
            "default_pose": None,   #与初始状态一致，打开
            "asset_cfg": [SceneEntityCfg("table")],
        },
    )

    randomize_franka_joint_state = EventTerm(
        func=franka_closedrawer_events.randomize_joint_by_gaussian_offset,
        mode="reset",
        params={
            "mean": 0.0,
            "std": 0.02,
...
@configclass
class FrankaCloseDrawerEnvCfg(CloseDrawerEnvCfg):
    ...
    def _marker_cfg(self, gelpad_prim: str) -> FOTSMarkerSimulatorCfg:
        return FOTSMarkerSimulatorCfg(
            ...
            frame_transformer_cfg=FrameTransformerCfg(
                ...
                target_frames=[
                    FrameTransformerCfg.FrameCfg(
                        prim_path="/World/envs/env_.*/Table/back_frame",
                        name="Table",
                    )
                ],
                debug_vis=False,
