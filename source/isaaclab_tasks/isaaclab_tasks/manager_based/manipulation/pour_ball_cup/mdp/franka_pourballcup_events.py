...
def enable_mesh_colliders(env, env_ids, asset_cfg, approximation: str = "meshSimplification"):
    """Enable mesh-based collision on all mesh prims under the given assets."""
    cfgs = _as_seq(asset_cfg)
    ...
    for cfg in cfgs:
        ...
        if prim_path is None and hasattr(asset, "cfg"):
            prim_path = asset.cfg.prim_path
        if prim_path is None and hasattr(env.scene, "cfg") and hasattr(env.scene.cfg, cfg.name):
            prim_path = getattr(env.scene.cfg, cfg.name).prim_path
        if prim_path is None:
            raise AttributeError(f"Could not resolve prim path for scene entity '{cfg.name}'.")

        prims = sim_utils.find_matching_prims(prim_path, stage=stage)
        for prim in prims:
