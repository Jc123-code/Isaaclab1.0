...
def _make_reference_point_cfg(name: str) -> RigidObjectCfg:
    return RigidObjectCfg(
        prim_path=f"{{ENV_REGEX_NS}}/Table/{name}",
        ...
        spawn=sim_utils.SphereCfg(
            ...
            rigid_props=sim_utils.RigidBodyPropertiesCfg(
                disable_gravity=False, #重力项
                kinematic_enabled=False, #动力学刚体
            ),
            collision_props=sim_utils.CollisionPropertiesCfg(
                collision_enabled=True,
            ),
            physics_material=sim_utils.materials.RigidBodyMaterialCfg(
                static_friction=0.4, #静摩擦系数。越大越“黏”，越不容易开始滑动。
                dynamic_friction=0.3, #动摩擦系数。物体已经在滑动时的阻力，越大越难滑动。
                restitution=0.0, #弹性系数（回弹）。0.0 代表不弹，1.0 代表完全弹起。
            ),
...
class ObjectTableSceneCfg(InteractiveSceneCfg):
    ...
    table_frame = FrameTransformerCfg(
        ...
        target_frames=[
            FrameTransformerCfg.FrameCfg(
                ...
                prim_path="{ENV_REGEX_NS}/Table/container",
                name="container",
                offset=OffsetCfg(
                    pos=(0.0, 0, 0),
                    rot=(1, 0.0, 0.0, 0.0), # align with end-effector frame
                ),
            ),
            FrameTransformerCfg.FrameCfg(
                # 手柄坐标系在哪里？它要固定附着在场景中已经存在的参考系，两者“相对位姿固定”
                prim_path="{ENV_REGEX_NS}/Table/ReferencePoint",
                name="Box",
                offset=OffsetCfg(
                    pos=(0.0, 0, 0),
                    rot=(1, 0.0, 0.0, 0.0), # align with end-effector frame
    ...
    ReferencePoint = _make_reference_point_cfg("ReferencePoint")
    ReferencePoint_1 = _make_reference_point_cfg("ReferencePoint_1")
    ReferencePoint_2 = _make_reference_point_cfg("ReferencePoint_2")
    ReferencePoint_3 = _make_reference_point_cfg("ReferencePoint_3")
    ReferencePoint_4 = _make_reference_point_cfg("ReferencePoint_4")
    ReferencePoint_5 = _make_reference_point_cfg("ReferencePoint_5")
    ReferencePoint_6 = _make_reference_point_cfg("ReferencePoint_6")
    ReferencePoint_7 = _make_reference_point_cfg("ReferencePoint_7")
    ReferencePoint_8 = _make_reference_point_cfg("ReferencePoint_8")
    ReferencePoint_9 = _make_reference_point_cfg("ReferencePoint_9")

    cup = RigidObjectCfg(
        prim_path="{ENV_REGEX_NS}/Table/cup",
        init_state=RigidObjectCfg.InitialStateCfg(
            pos=(0.47, 0.0, 1.05),
            rot=(0, 0, 0.70711, 0.70711)
        ),
        spawn=sim_utils.UsdFileCfg(
            usd_path=f"{ISAAC_NUCLEUS_DIR}/Props/Mounts/mine_assets/pourballcup/Cup1.usdz",
            scale=(0.1, 0.1, 0.1),
            rigid_props=sim_utils.RigidBodyPropertiesCfg(
    ...
    container = RigidObjectCfg(
        ...
        spawn=sim_utils.UsdFileCfg(
            usd_path=f"{ISAAC_NUCLEUS_DIR}/Props/Mounts/mine_assets/pourballcup/5oz_Ice_Cream_Cup5.usdz",
            ...
            # collision is applied at startup via mesh collision API (see EventCfg)
            # visual_material=sim_utils.PreviewSurfaceCfg(
            #     diffuse_color=(1.0, 1.0, 0.0),  # 黄色
            #     metallic=0.1,
            #     roughness=0.4
...
class TerminationsCfg:
    ...
    success = DoneTerm(func=mdp.cup_success)


@configclass
class PourBallCupEnvCfg(ManagerBasedRLEnvCfg):
