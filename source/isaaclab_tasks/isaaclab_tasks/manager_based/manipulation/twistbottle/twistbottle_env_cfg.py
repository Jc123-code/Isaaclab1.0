...
class ObjectTableSceneCfg(InteractiveSceneCfg):
    ...
    #     prim_path="{ENV_REGEX_NS}/Table/bottle",
    #     init_state=RigidObjectCfg.InitialStateCfg(
    #         pos=(-0.4, 0.7, 3.3),
    #         rot=(1, 0, 0, 0)
    #     ),
    #     spawn=sim_utils.UsdFileCfg(
    #         usd_path=f"{ISAAC_NUCLEUS_DIR}/Props/Mounts/mine_assets/Medicine_Pill_Bottle/Medicine_Pill_Bottle_With_Label/bottle2.usdz",
    ...
    #             mass=0.01,
    #         ),
    #         collision_props=sim_utils.CollisionPropertiesCfg(  # ✅ 添加碰撞属性
    #             collision_enabled=True,
    #         ),
    #         # visual_material=sim_utils.PreviewSurfaceCfg(  # ✅ 添加可视化材质
    #         #     diffuse_color=(1.0, 0.6, 0.2),  # 橙黄色瓶子
    #         #     metallic=0.1,
    #         #     roughness=0.4,
    #         # ),
    #     ),
    ...
    bottle = ArticulationCfg(
        ...
        # 载入 USD（保持你的缩放）
        spawn=UsdFileCfg(
            usd_path=f"{ISAAC_NUCLEUS_DIR}/Props/Mounts/mine_assets/Medicine_Pill_Bottle/Medicine_Pill_Bottle_With_Label/bottle3.usdz",
            scale=(0.01, 0.01, 0.01),
        ...
        # 初始状态（保持你原来的位姿），并给关节一个初始位置
        init_state=ArticulationCfg.InitialStateCfg(
            pos=(-0.4, 0.7, 3.3),
            rot=(1, 0, 0, 0),
            joint_pos={
                "cap_joint": 0.0,   # 假设瓶盖关节名为 cap_joint（旋转）
            },
        ),

        # 关节的驱动/限制（仿照 table 的 actuators 写法）
        actuators={
            "cap": ImplicitActuatorCfg(
                joint_names_expr=["cap_joint"],
                # 小物体的瓶盖，不需要太大扭矩；这些数值只是合理起点，可再按手感微调
                effort_limit=0.25,    # ~0.25 N·m 扭矩上限：拧开/拧紧的“阻力”
                velocity_limit=30.0,  # 角速度上限（rad/s）
                stiffness=2.0,        # 关节弹性（0 更“自由转”，>0 有回正/弹性）
                damping=0.15,         # 阻尼，转动的“涩感”
            ),
        }
    )
...
@configclass
class TwistBottleEnvCfg(ManagerBasedRLEnvCfg):
