...
def reset_switch_pose_with_randomized_y(
    env: ManagerBasedEnv,
    env_ids: torch.Tensor,
    y_values: tuple[float, float],
    default_pose: Union[torch.Tensor, Sequence[torch.Tensor]] | None,
    asset_cfg: Union[SceneEntityCfg, Sequence[SceneEntityCfg]] = SceneEntityCfg("switch"),
):
    """Reset switch root pose and joint state, with root y sampled from two discrete values."""
    ...
    cfgs = _as_seq(asset_cfg)
    if len(cfgs) != 1:
        raise ValueError("reset_switch_pose_with_randomized_y expects exactly one switch asset.")

    switch: Articulation = env.scene[cfgs[0].name]

    root_state = switch.data.default_root_state[env_ids].clone()
    root_state[:, :3] += env.scene.env_origins[env_ids]

    sampled_y = torch.randint(0, 2, (env_ids.numel(),), device=env.device)
    y_candidates = torch.tensor(y_values, device=env.device, dtype=root_state.dtype)
    root_state[:, 1] = y_candidates[sampled_y] + env.scene.env_origins[env_ids, 1]

    switch.write_root_pose_to_sim(root_state[:, :7], env_ids)
    switch.write_root_velocity_to_sim(root_state[:, 7:], env_ids)
    switch._data.root_state_w[env_ids] = root_state.clone()

    joint_pos = switch.data.default_joint_pos[env_ids].clone()
    joint_vel = switch.data.default_joint_vel[env_ids].clone()
    if default_pose is not None:
        pose = torch.as_tensor(default_pose, device=env.device).view(1, -1)
        dof = switch.data.default_joint_pos.shape[-1]
        assert pose.shape[-1] == dof
        joint_pos = pose.repeat(env_ids.numel(), 1)

    switch.set_joint_position_target(joint_pos, env_ids=env_ids)
    switch.set_joint_velocity_target(joint_vel, env_ids=env_ids)
    switch.write_joint_state_to_sim(joint_pos, joint_vel, env_ids=env_ids)
...
def reset_pose_to_default(env, env_ids, default_pose, asset_cfg):
    ...
    # 允许 default_pose 为 None，表示使用 asset 的 init_state
    if default_pose is None:
        default_pose = [None] * len(asset_cfg)
    elif isinstance(default_pose, torch.Tensor):
        default_pose = [default_pose]
    elif isinstance(default_pose, list):
        if not all((p is None) or isinstance(p, torch.Tensor) for p in default_pose):
            raise TypeError("Each default_pose element must be a torch.Tensor or None.")
    else:
        raise TypeError("default_pose must be a Tensor, list of Tensors, or None.")
    ...
    for i, cfg in enumerate(asset_cfg):
        ...
        if pose is None:
            # If asset has no cfg/init_state (e.g., AssetBase/XformPrim), skip reset.
            if not hasattr(asset, "cfg") or asset.cfg is None or not hasattr(asset.cfg, "init_state"):
                continue
            init_pos = torch.tensor(asset.cfg.init_state.pos, device=env.device).unsqueeze(0)
            init_rot = torch.tensor(asset.cfg.init_state.rot, device=env.device).unsqueeze(0)
            pose = torch.cat([init_pos, init_rot], dim=-1)

        # If ReferencePoint is being reset, use its local init_state in Table frame and convert to world.
        if cfg.name == "ReferencePoint":
            env_ids_t = torch.as_tensor(env_ids, device=pose.device, dtype=torch.long)
            table: Articulation = env.scene["table"]
            table_root_state = table.data.root_state_w[env_ids_t, :7]
            table_pos = table_root_state[:, 0:3]
            table_quat = table_root_state[:, 3:7]

            init_pos = torch.tensor(asset.cfg.init_state.pos, device=pose.device).unsqueeze(0)
            init_rot = torch.tensor(asset.cfg.init_state.rot, device=pose.device).unsqueeze(0)
            init_pos = init_pos.repeat(env_ids_t.numel(), 1)
            init_rot = init_rot.repeat(env_ids_t.numel(), 1)

            world_pos, world_quat = math_utils.combine_frame_transforms(table_pos, table_quat, init_pos, init_rot)
            pose = torch.cat([world_pos, world_quat], dim=-1)
