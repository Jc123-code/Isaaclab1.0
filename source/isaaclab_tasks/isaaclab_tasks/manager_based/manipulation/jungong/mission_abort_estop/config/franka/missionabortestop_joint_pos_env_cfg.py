...
from isaaclab_tasks.manager_based.manipulation.jungong.mission_abort_estop import mdp
from isaaclab_tasks.manager_based.manipulation.jungong.mission_abort_estop.mdp import franka_missionabortestop_events
from isaaclab_tasks.manager_based.manipulation.jungong.mission_abort_estop.missionabortestop_env_cfg import (
    MissionAbortEstopEnvCfg,
)
...
class EventCfg:
    ...
    init_franka_arm_pose = EventTerm(
        func=franka_missionabortestop_events.set_default_joint_pose,
        mode="startup",
        params={
    ...
    init_table_joint_pose = EventTerm(
        ...
        mode="startup",
        params={
            # Table has no joints; use empty joint pose.
            "default_pose": [[]],
    ...
    reset_table_joint_pose = EventTerm(
        func=franka_missionabortestop_events.reset_to_default_joint_pose,
        mode="reset",
        params={
            "default_pose": None,
            "asset_cfg": [SceneEntityCfg("table")],
    ...
    randomize_franka_joint_state = EventTerm(
        func=franka_missionabortestop_events.randomize_joint_by_gaussian_offset,
        mode="reset",
        params={
            "mean": 0.0,
            "std": 0.02,
    ...
    reset_button_pose = EventTerm(
        func=franka_missionabortestop_events.reset_pose_to_default_with_randomized_y,
        mode="reset",
        params={
            "default_pose": torch.tensor([
                # 0.52077, -0.5, 1.2, 0, 0, 0.70711, 0.70711,   # 蓝书位置 + 姿态
                0.52077, -0.5, 1.2, 1, 0, 0, 0,   # 蓝书位置 + 姿态
                0, 0, 0, 0, 0, 0              # 静止
            ], device="cuda"),
            "y_values": (-0.5, 0.5),
...
@configclass
class FrankaMissionAbortEstopEnvCfg(MissionAbortEstopEnvCfg):
    ...
    def _marker_cfg(self, gelpad_prim: str) -> FOTSMarkerSimulatorCfg:
        return FOTSMarkerSimulatorCfg(
            ...
            frame_transformer_cfg=FrameTransformerCfg(
                ...
                target_frames=[
                    FrameTransformerCfg.FrameCfg(
                        prim_path="/World/envs/env_.*/Table/ReferencePoint",
                        name="ReferencePoint",
                    )
                ],
                debug_vis=False,
