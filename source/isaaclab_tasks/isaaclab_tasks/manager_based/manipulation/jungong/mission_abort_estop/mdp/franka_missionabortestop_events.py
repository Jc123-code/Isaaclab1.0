...
def reset_pose_to_default_with_randomized_y(
    ...
    """Reset rigid objects to the given default pose while sampling local y from two values."""
    ...
    num_envs = len(env_ids)
    root_state = pose.repeat(num_envs, 1)
    sampled_y = torch.randint(0, 2, (num_envs,), device=env.device)
    y_candidates = torch.tensor(y_values, device=env.device, dtype=root_state.dtype)
    root_state[:, 1] = y_candidates[sampled_y]
