...
class ObjectTableSceneCfg(InteractiveSceneCfg):
    ...
    table_frame = FrameTransformerCfg(
        # 以 door 作为 source frame
        prim_path="{ENV_REGEX_NS}/container/Meshes/door",
        debug_vis=False,
        target_frames=[
    ...
    container = ArticulationCfg(
        # USD 场景中的 prim 路径。
        # {ENV_REGEX_NS} 是每个并行环境的命名空间占位符，
        # 因此该容器会在每个环境中以 ".../container" 的路径被创建。
        prim_path="{ENV_REGEX_NS}/container",

        # 物体的初始状态配置：包括初始位置和朝向。
        init_state=ArticulationCfg.InitialStateCfg(
            # 初始位置 (x, y, z)
            # 将容器放置在环境局部坐标中的 (0.5, -0.3, 1.2) 位置。
            pos=(3.8, 0, 1.2),
        ...
        spawn=sim_utils.UsdFileCfg(
            ...
            # 以及可能的碰撞/刚体/关节信息。
            usd_path=f"{ISAAC_NUCLEUS_DIR}/Props/Mounts/mine_assets/jungong/rotate_handle_lock/new_4.usdz",
...
class RotateHandleLockEnvCfg(ManagerBasedRLEnvCfg):
    """Configuration for the rotate_handle_lock environment."""
