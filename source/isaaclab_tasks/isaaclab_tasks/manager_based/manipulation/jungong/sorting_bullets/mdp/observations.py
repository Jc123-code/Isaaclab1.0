...
def rel_ee_drawer_distance(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    bullet_762: RigidObject = env.scene["bullet_762"]
    return bullet_762.data.root_state_w[:, :3] - ee_tf_data.target_pos_w[..., 0, :]
...
def handle_grasped(
    ...
    ee_tf_data: FrameTransformerData = env.scene["ee_frame_right"].data
    bullet_762: RigidObject = env.scene["bullet_762"]
    dist = torch.norm(bullet_762.data.root_state_w[:, :3] - ee_tf_data.target_pos_w[..., 0, :], dim=1)
...
def drawer_dragged(
    ...
    # 查看两个目标物体在 x 方向的偏差
    dist_x = torch.abs(bullet_762.data.root_state_w[:, 0] - bullet_9.data.root_state_w[:, 0])
