...
from isaaclab_tasks.manager_based.manipulation.jungong.plant_redflag import mdp
from isaaclab_tasks.manager_based.manipulation.jungong.plant_redflag.mdp import franka_plant_redflag_events
from isaaclab_tasks.manager_based.manipulation.jungong.plant_redflag.plant_redflag_env_cfg import PlantredflagEnvCfg
...
class EventCfg:
    ...
    weld_sandbox_to_table = EventTerm(
        func=franka_plant_redflag_events.create_fixed_joint_between_assets,
        mode="startup",
        params={
    ...
    init_franka_arm_pose = EventTerm(
        func=franka_plant_redflag_events.set_default_joint_pose,
        mode="startup",
        params={
    ...
    randomize_franka_joint_state = EventTerm(
        func=franka_plant_redflag_events.randomize_joint_by_gaussian_offset,
        mode="reset",
        params={
            "mean": 0.0,
            "std": 0.02,
    ...
    reset_sandbox_pose = EventTerm(
        func=franka_plant_redflag_events.reset_pose_to_default,
        mode="reset",
        params={
            "default_pose": torch.tensor([
                0.61, -0.16692, 1.03168, 0.70651, 0.70769, 0.00172, 0.00402,
                0, 0, 0, 0, 0, 0              # 静止
            ], device="cuda"),
            "asset_cfg": [SceneEntityCfg("sandbox")],
        },
    )

    reset_redflag_pose = EventTerm(
        func=franka_plant_redflag_events.reset_pose_to_default,
        mode="reset",
        params={
            "default_pose": torch.tensor([
                0.54085, -0.50117, 1.03, 0.65683, 0.65591, 0.26176, 0.26427,   
                0, 0, 0, 0, 0, 0              # 静止
            ], device="cuda"),
...
@configclass
class FrankaPlantredflagEnvCfg(PlantredflagEnvCfg):
