...
from isaaclab_tasks.manager_based.manipulation.jungong.get_place_bandage import mdp
from isaaclab_tasks.manager_based.manipulation.jungong.get_place_bandage.mdp import franka_getplace_bandage_events
from isaaclab_tasks.manager_based.manipulation.jungong.get_place_bandage.getplace_bandage_env_cfg import GetPlacebandageEnvCfg
...
class EventCfg:
    ...
    init_franka_arm_pose = EventTerm(
        func=franka_getplace_bandage_events.set_default_joint_pose,
        mode="startup",
        params={
    ...
    reset_table_joint_pose = EventTerm(
        func=franka_getplace_bandage_events.reset_to_default_joint_pose,
        mode="reset",
        params={
            "default_pose": None,
            "asset_cfg": [SceneEntityCfg("table")],
    ...
    randomize_franka_joint_state = EventTerm(
        func=franka_getplace_bandage_events.randomize_joint_by_gaussian_offset,
        mode="reset",
        params={
            "mean": 0.0,
            "std": 0.02,
    ...
    reset_first_aid_kit_pose = EventTerm(
        func=franka_getplace_bandage_events.reset_pose_to_default,
        mode="reset",
        params={
            "default_pose": torch.tensor([
                0.8, 0.15, 1.05, 0,0,-0.70711,-0.70711,   
                0, 0, 0, 0, 0, 0              # 静止
            ], device="cuda"),
            "asset_cfg": [SceneEntityCfg("first_aid_kit")],
        },
    )

    reset_bandage_pose = EventTerm(
        func=franka_getplace_bandage_events.reset_bandage_pose_to_default,
        mode="reset",
        params={
            "pose_range": {
                "x": (0.425, 0.425),   # x 固定
                "y": (0.434, 0.634),   # y 在 0.534±0.1 范围内随机
                "z": (1.0, 1.0),       # z 固定
            },
            "fixed_quat": torch.tensor([0.5, 0.5, -0.5, -0.5]),  # 固定四元数
            "asset_cfg": [SceneEntityCfg("bandage")],
        },
    )
...
@configclass
class FrankaGetPlacebandageEnvCfg(GetPlacebandageEnvCfg):
    ...
    def _marker_cfg(self, gelpad_prim: str) -> FOTSMarkerSimulatorCfg:
        return FOTSMarkerSimulatorCfg(
            ...
            frame_transformer_cfg=FrameTransformerCfg(
                ...
                target_frames=[
                    FrameTransformerCfg.FrameCfg(
                        prim_path="/World/envs/env_.*/Table/bandage",
                        name="bandage",
                    )
                ],
                debug_vis=False,
