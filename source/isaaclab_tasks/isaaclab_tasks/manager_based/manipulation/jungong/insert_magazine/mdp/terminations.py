...
def insert_magazine_success(
    ...
    magazine_pos_w = drawer_tf_data.target_pos_w[..., 0, :]

    center = torch.tensor(
        target_center,
        device=magazine_pos_w.device,
        dtype=magazine_pos_w.dtype,
    )

    # 各轴距离
    dx = torch.abs(magazine_pos_w[..., 0] - center[0])
    dy = torch.abs(magazine_pos_w[..., 1] - center[1])
    dz = torch.abs(magazine_pos_w[..., 2] - center[2])
    ...
    # 初始化 timer 状态缓存
    # ---------------------------
    timer_name = "_insert_magazine_success_timer"
    ...
    if hasattr(env, "episode_length_buf"):
        prev_len_name = "_insert_magazine_prev_episode_length_buf"
