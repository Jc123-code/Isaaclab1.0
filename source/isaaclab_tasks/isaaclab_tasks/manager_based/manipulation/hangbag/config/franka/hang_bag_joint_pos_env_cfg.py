...
from isaaclab_tasks.manager_based.manipulation.hangbag import mdp
from isaaclab_tasks.manager_based.manipulation.hangbag.mdp import franka_hang_bag_events
from isaaclab_tasks.manager_based.manipulation.hangbag.hang_bag_env_cfg import HangBagEnvCfg
...
class EventCfg:
    ...
    init_franka_arm_pose = EventTerm(
        func=franka_hang_bag_events.set_default_joint_pose,
        mode="startup",
        params={
    ...
    reset_table_joint_pose = EventTerm(
        func=franka_hang_bag_events.reset_to_default_joint_pose,
        mode="reset",
        params={
            "default_pose": None,
            "asset_cfg": [SceneEntityCfg("table")],
    ...
    randomize_franka_joint_state = EventTerm(
        func=franka_hang_bag_events.randomize_joint_by_gaussian_offset,
        mode="reset",
        params={
            "mean": 0.0,
            "std": 0.02,
    ...
    reset_bag_pose = EventTerm(
        func=franka_hang_bag_events.reset_pose_to_default,
        mode="reset",
        params={
            "default_pose": torch.tensor([
                0.48719, 0.43902, 1.02, 0.31128, 0.31128, 0.63491, 0.63491,   
                0, 0, 0, 0, 0, 0              # 静止
            ], device="cuda"),
...
@configclass
class FrankaHangBagEnvCfg(HangBagEnvCfg):
