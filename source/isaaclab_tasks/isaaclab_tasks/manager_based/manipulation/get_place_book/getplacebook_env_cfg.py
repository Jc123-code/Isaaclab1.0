...
class ObjectTableSceneCfg(InteractiveSceneCfg):
    ...
    """
    book_red = AssetBaseCfg(
        prim_path="{ENV_REGEX_NS}/Table/book_red",
        init_state=AssetBaseCfg.InitialStateCfg(
            pos=(-0.4, 0.7, 3.3),
            rot=(0.70711, 0, 0, 0.70711)
        ),
        spawn=UsdFileCfg(
            usd_path=f"{ISAAC_NUCLEUS_DIR}/Props/Mounts/mine_assets/Book3.usdz",
            scale=(2.0, 2.0, 2.0),
    ...
    book_red = RigidObjectCfg(
        prim_path="{ENV_REGEX_NS}/Table/book_red",
        init_state=RigidObjectCfg.InitialStateCfg(
            pos=(0.25, 1.8, 3.2),#0.5, 1.8, 3.2
    ...
    book_blue = RigidObjectCfg(
        prim_path="{ENV_REGEX_NS}/Table/book_blue",
        init_state=RigidObjectCfg.InitialStateCfg(
            pos=(0.25, 1.5, 3.2),#0.5, 1.5, 3.2
            rot=((0.5, 0.5, 0.5, 0.5))
        ),
        spawn=sim_utils.UsdFileCfg(
            ...
            rigid_props=sim_utils.RigidBodyPropertiesCfg(
                ...
                kinematic_enabled=False, # 是否由物理引擎控制（False 表示自由体）
            ),
            mass_props=sim_utils.MassPropertiesCfg(mass=0.1),
            # ✅ 碰撞属性：否则书本没法被检测、抓取
    ...
    # 获取相对于桌子的变动的一些位置姿态（变换关系）,便于求得物体初始位姿
    table_frame = FrameTransformerCfg(
        ...
        target_frames=[
            ...
            FrameTransformerCfg.FrameCfg(
                prim_path="{ENV_REGEX_NS}/Table/right_door",
                name="right_door_reference",
                offset=OffsetCfg(
                    pos=(0.0, -0.5, 0),
                    rot=(1.0, 0.0, 0.0, 0.0), # align with end-effector frame
                ),
...
class TerminationsCfg:
    ...
    success = DoneTerm(func=mdp.getplace_book)


@configclass
class GetPlaceBookEnvCfg(ManagerBasedRLEnvCfg):
