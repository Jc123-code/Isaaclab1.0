...
#设置默认关节姿态：这个函数将指定资产的默认关节位置更新为你提供的值。它会将单个姿态复制到所有并行环境中。简单来说，它是"记忆"设置，告诉系统下次重置时应该用什么姿态。
...
#这个函数把指定资产（可能是多个）重置到给定的 default_pose（或资产内置的默认）并把该姿态写到每个并行环境（env.num_envs）
...
# def reset_book_pose_to_default(env, env_ids, default_pose, asset_cfg):
#     """Reset the book rigid object to its default pose and zero velocity."""
#     # 获取目标物体（书本）资产
#     asset = env.scene[asset_cfg[0].name]

#     # 如果 default_pose 是单个向量 (13,) 或 (7,)
#     if default_pose.ndim == 1:
#         default_pose = default_pose.unsqueeze(0)

#     # 检查是否只包含位置+姿态 (7维)，若是则补上6个0速度
#     if default_pose.shape[-1] == 7:
#         zeros = torch.zeros((default_pose.shape[0], 6), device=default_pose.device)
#         default_pose = torch.cat([default_pose, zeros], dim=-1)

#     # 针对当前重置的 env_ids，构造对应形状
#     num_envs = len(env_ids)
#     if default_pose.shape[0] == 1:
#         root_state = default_pose.repeat(num_envs, 1)
#     elif default_pose.shape[0] == num_envs:
#         root_state = default_pose
#     else:
#         raise ValueError(
#             f"default_pose shape {default_pose.shape} incompatible with env_ids length {num_envs}"
#         )

#     # 写入仿真：位置+姿态+速度
#     asset.write_root_state_to_sim(root_state, env_ids=env_ids)

#     # 同步 root_state 缓存（可选但推荐）
#     asset._data.root_state_w[env_ids] = root_state.clone()

#     print(f"[Reset] Book pose reset for env_ids={env_ids.tolist()} to default pose.")


def reset_book_pose_to_default(
    env: ManagerBasedEnv,
    env_ids: torch.Tensor,
    ...
    print(f"[Reset] object pose randomized for env_ids={env_ids.tolist()}")
