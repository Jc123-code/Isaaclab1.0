...
from isaaclab_tasks.manager_based.manipulation.getbluebook_bimanual import mdp
from isaaclab_tasks.manager_based.manipulation.getbluebook_bimanual.mdp import franka_getbluebook_events
from isaaclab_tasks.manager_based.manipulation.getbluebook_bimanual.getbluebook_env_cfg import GetBlueBookEnvCfg
...
class EventCfg:
    ...
    init_franka_arm_pose = EventTerm(
        func=franka_getbluebook_events.set_default_joint_pose,
        mode="startup",
        params={
...
#重置逻辑
    ...
    reset_table_joint_pose = EventTerm(
        func=franka_getbluebook_events.reset_to_default_joint_pose,
        mode="reset",
        params={
            "default_pose": None,
            "asset_cfg": [SceneEntityCfg("table")],
    ...
    randomize_franka_joint_state = EventTerm(
        func=franka_getbluebook_events.randomize_joint_by_gaussian_offset,
        mode="reset",
        params={
            "mean": 0.0,
            "std": 0.02,
...
#目标物体：
    ...
    reset_red_book_pose = EventTerm(
        func=franka_getbluebook_events.reset_book_pose_to_default,
        mode="reset",
        params={
            "pose_range": {
                "x": (0.8, 0.8),           # x 固定
...
@configclass
class FrankaGetBlueBookEnvCfg(GetBlueBookEnvCfg):
    ...
    def _marker_cfg(self, gelpad_prim: str) -> FOTSMarkerSimulatorCfg:
        return FOTSMarkerSimulatorCfg(
            ...
            frame_transformer_cfg=FrameTransformerCfg(
                ...
                target_frames=[
                    FrameTransformerCfg.FrameCfg(
                        prim_path="/World/envs/env_.*/Table/book_blue",
                        name="book_blue",
                    )
                ],
                debug_vis=False,
