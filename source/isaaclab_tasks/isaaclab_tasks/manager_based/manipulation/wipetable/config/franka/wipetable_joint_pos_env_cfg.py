...
from isaaclab_tasks.manager_based.manipulation.wipetable import mdp
from isaaclab_tasks.manager_based.manipulation.wipetable.mdp import franka_wipetable_events
from isaaclab_tasks.manager_based.manipulation.wipetable.wipetable_env_cfg import WipeTableEnvCfg
...
class EventCfg:
    ...
    init_franka_arm_pose = EventTerm(
        func=franka_wipetable_events.set_default_joint_pose,
        mode="startup",
        params={
    ...
    reset_table_joint_pose = EventTerm(
        func=franka_wipetable_events.reset_to_default_joint_pose,
        mode="reset",
        params={
            "default_pose": None,
            "asset_cfg": [SceneEntityCfg("table")],
    ...
    reset_towel_red_pose = EventTerm(
        func=franka_wipetable_events.reset_towel_pose_to_default,
        mode="reset",
        params={
            "default_pose": torch.tensor([
                0.5, -0.2, 1.5, 1.0, 0, 0, 0,  # 抹布位置 + 姿态
                0, 0, 0, 0, 0, 0              # 静止
            ], device="cuda"),
            "asset_cfg": [SceneEntityCfg("towel")],
        },
    )

    randomize_franka_joint_state = EventTerm(
        func=franka_wipetable_events.randomize_joint_by_gaussian_offset,
        mode="reset",
        params={
            "mean": 0.0,
            "std": 0.02,
...
@configclass
class FrankaWipeTableEnvCfg(WipeTableEnvCfg):
