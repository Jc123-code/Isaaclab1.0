...
def _align_pose_to_dof(pose_1xdof: torch.Tensor, default_1xdof: torch.Tensor) -> torch.Tensor:
    """Align a pose vector to articulation DoF by truncate/pad.

    If pose length is larger than DoF, keep the first DoF entries.
    If pose length is smaller than DoF, pad the tail with current default values.
    """
    target_dof = default_1xdof.shape[-1]
    src_dof = pose_1xdof.shape[-1]
    if src_dof == target_dof:
        return pose_1xdof
    if src_dof > target_dof:
        return pose_1xdof[:, :target_dof]
    # src_dof < target_dof: pad with existing default tail
    return torch.cat([pose_1xdof, default_1xdof[:, src_dof:]], dim=-1)
...
def set_default_joint_pose(
    ...
    if default_pose is not None:
        ...
        for pose, cfg  in zip(poses, cfgs):
            ...
            pose = _align_pose_to_dof(pose, asset.data.default_joint_pos[:1])
...
def reset_to_default_joint_pose(
    ...
    if default_pose is not None:
        ...
        for pose, cfg  in zip(poses, cfgs):
            ...
            pose = torch.as_tensor(pose, device=env.device).view(1, -1)  # (1, DoF)
            pose = _align_pose_to_dof(pose, asset.data.default_joint_pos[:1])
            q = pose.repeat(env_ids_t.numel(), 1)
            qd = torch.zeros_like(q)
            asset.set_joint_position_target(q, env_ids=env_ids_t)
            asset.set_joint_velocity_target(qd, env_ids=env_ids_t)
            asset.write_joint_state_to_sim(q, qd, env_ids=env_ids_t)
    else:
        for cfg  in cfgs:
            asset: Articulation = env.scene[cfg.name]
            q = asset.data.default_joint_pos[env_ids_t].clone()
            q = _align_pose_to_dof(q, asset.data.default_joint_pos[:1])
            qd = torch.zeros_like(q)
            asset.set_joint_position_target(q, env_ids=env_ids_t)
            asset.set_joint_velocity_target(qd, env_ids=env_ids_t)
            asset.write_joint_state_to_sim(q, qd, env_ids=env_ids_t)
